...
async function buildShortWorkbook(
  rows: ProductRow[],
  results: Record<string, CalcResult | { error: string }>,
): Promise<Buffer> {
...
export function exportRoutes(): Hono {
  ...
  app.post("/xlsx", async (c) => {
    ...
    if (kind !== "short" && kind !== "full") {
      return c.json({ error: "kind must be 'short' or 'full'" }, 400);
    }
    ...
    let baseName: string;
    if (kind === "short") {
      buf = await buildShortWorkbook(rows, results);
      baseName = `ozon-расчёт-${nowFileStamp()}.xlsx`;
    } else {
      if (!body.taxSettings || typeof body.taxSettings !== "object") {
        return c.json({ error: "taxSettings required for kind='full'" }, 400);
      }
      buf = await buildFullWorkbook(
        rows,
        results,
        body.taxSettings as TaxSettings,
      );
      baseName = `ozon-полный-${nowFileStamp()}.xlsx`;
    }
